...
public class MapMode extends GameMode {
  ...
  private static final Logger logger = Logger.getLogger(MapMode.class.getName());
  ...
  public void join(User user) {
    ...
    ppsm.playerPositionMessage = ppms.toArray(new PlayerPositionMessage[ppms.size()]);
  ...
  public void received(PlayerMove customType) {
    ...
    HashSet<Player> inRange = u.getZone().getPlayersInRange(u, Constants.VISIBILITY_RANGE);
    u.setPosition(p);
    inRange = u.getZone().getPlayersInRange(u, Constants.VISIBILITY_RANGE, inRange);
    if (inRange.size() <= 0)
      return;
    ArrayList<PlayerPositionMessage> ppms = new ArrayList<PlayerPositionMessage>();
    for (Player user : inRange) {
